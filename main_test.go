...
func TestStorageFileQiniu(t *testing.T) {
	TestStorageLocal(t)
	tLocal := New("", map[string]string{
		"bucket":    "",
		"accessKey": "",
		"secretKey": "",
		"domain":    "",
	...
	err = tLocal.WriteFile(context.Background(), path, key, map[string]any{})
	if err != nil {
		t.Error("write failed:" + err.Error())
	}

	str, err := tLocal.Read(context.Background(), path)
	if err != nil {
		t.Error("read failed:" + err.Error())
	} else {
		t.Log("read success:" + str)
	}

	err = tLocal.Delete(context.Background(), path)
	if err != nil {
		t.Error("write failed:" + err.Error())
	}
}
